...
class ZHIPUAIBot(Bot, ZhipuAIImage):
    ...
    def reply(self, query, context=None):
        ...
        if context.type == ContextType.TEXT:
            ...
            logger.debug(f"[ZHIPU_AI] 使用会话ID: {session_id}")
            ...
            # 获取会话前记录会话数量
            session_count_before = len(self.sessions.sessions)
            logger.debug(f"[ZHIPU_AI] 当前会话数量: {session_count_before}")
            
            session = self.sessions.session_query(query, session_id)
            logger.debug("[ZHIPU_AI] 会话消息数量: {}".format(len(session.messages)))
            logger.debug("[ZHIPU_AI] 会话消息: {}".format(session.messages))
            ...
            # 获取会话后记录会话数量
            session_count_after = len(self.sessions.sessions)
            logger.debug(f"[ZHIPU_AI] 回复后会话数量: {session_count_after}")
